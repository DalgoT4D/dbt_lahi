...
-- LEFT JOIN {{ ref('vt_school_sectors') }} VSS ON SC.SchoolId = VSS.SchoolId
-- LEFT JOIN {{ ref('vtp_sectors') }} VS ON SE.sectorid = VS.sectorid
...
-- LEFT JOIN {{ ref('sector_job_roles') }} SJR ON SE.SectorId = SJR.SectorId
-- LEFT JOIN {{ ref('job_roles')}} JR ON SJR.jobroleid = JR.jobroleid
